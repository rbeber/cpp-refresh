...
int main() {
    ...
    const std::map<std::string, std::string> map_seasons = {std::pair{"spring","STL guru"},
                                              std::pair{"summer","C++ expert"},
                                              std::pair{"autumn","coding beast"},
                                              std::pair{"winter","software design hero"}};
    //Use the modulo division by the size of the array of your adjectives on the length 
    //of the provided <NAME> to get an index that you can use to get an adjective 
    //from your array of adjectives
    //<NAME>, the <ADJECTIVE> <NOUN> that <ENDING>
   
    cout << user_name +
            ", the "s +
            vect_adjectives[vect_adjectives.size()%sizeof(user_name)] +
            map_seasons.at(birth_season)+
            " that "s +
            vect_endings[vect_endings.size()%sizeof(user_name)] 
            << endl;     
    return 0;
}